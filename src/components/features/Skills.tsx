    ...
    return (
        <SectionWrapper id="skills">
            ...
            <InfiniteSlider durationOnHover={700} duration={50} gap={24} className='py-space32'>
                {skillIcons.map((item) => (
                    <div key={item} className="mx-3" title={item.split(':')[1]}>
                        <Icon icon={item} className='text-7xl text-secondary-100' />
                    </div>
                ))}
            </InfiniteSlider>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 pt-space8'>
                {featureList.map((item) =>
                    <Card key={`featureCard` + item.id} className='z-10 w-full'>
                        <div className='h-[18rem] px-space24 py-space16 text-100 flex flex-col justify-center items-between gap-space16 hover:-translate-y-4 duration-300'>
                            <div className="flex items-center gap-3">
                                <Icon icon={item.icon} className='text-4xl text-secondary-100' />

                                <p className="text-lg font-semibold leading-10">{item.title}</p>
                            </div>
